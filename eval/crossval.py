...
def stanza_process(tokens):
    ...
    stanza_preds = [
        tag[:2] + "PLACE" if tag[2:] in ["GPE", "LOC"] else "O" for tag in stanza_preds
    ]
    return stanza_preds
...
split_size = len(conll) // 5
conll_subsets = [conll[x : x + split_size] for x in range(0, len(conll), split_size)]
